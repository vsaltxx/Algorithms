...
void Stack_Init( Stack *stack ) {
    ...
    stack->topIndex = -1;                                       // set the top of the stack to -1 (empty stack)
    return;
...
bool Stack_IsEmpty( const Stack *stack ) {
    ...
    return (stack->topIndex == -1); // return 1 if stack is empty (true), 0 if not (false)
}
...
bool Stack_IsFull( const Stack *stack ) {
    ...
    return (stack->topIndex == STACK_SIZE - 1); // the first element is on position 0, so the last element is on position STACK_SIZE - 1
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    ...
    *dataPtr = stack->array[stack->topIndex]; // copy the top element of the stack to dataPtr
    return;
}
...
void Stack_Push( Stack *stack, char data ) {
    ...
    stack->array[stack->topIndex] = data;   // insert the data to the top of the stack
    return;
}
...
void Stack_Dispose( Stack *stack ) {
    ...
    free(stack->array);     // free the memory allocated for the stack
